...
import { Terminal, Lock, Mail, User as UserIcon, ArrowRight, AlertCircle, Sparkles, Play } from "lucide-react";
      ...
      <div className="w-full max-w-md bg-[#111113] border border-[#29292D] rounded-lg p-7 shadow-2xl space-y-6">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-[11px] text-[#77777D]">
            API Engineering Workbench • Create developer workspace
          </p>
        </div>
        ...
        {onEnterDemo && (
          <div className="p-3 bg-[#C98A3D]/10 border border-[#C98A3D]/30 rounded space-y-2">
            ...
            <p className="text-[11px] text-[#77777D] leading-relaxed">
              Skip registration and evaluate the live workbench directly using the Atlas Workflow API demo dataset.
            </p>
            <button
              onClick={onEnterDemo}
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          <div>
            ...
            <div className="relative">
              <UserIcon className="w-3.5 h-3.5 text-[#77777D] absolute left-3 top-3" />
              <input
                type="text"
                required
                value={name}
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full flex items-center justify-center space-x-2 py-2 rounded bg-[#161619] hover:bg-[#202024] border border-[#29292D] hover:border-[#C98A3D]/50 text-xs font-semibold text-[#D8D8DC] transition-colors disabled:opacity-50 mt-1"
          >
            <span>{isLoading ? "Provisioning..." : "Create Account"}</span>
