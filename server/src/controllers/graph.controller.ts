...
export const buildGraphController = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (req.body && (req.body.canonicalModel || req.body.metadata)) {
      ...
      const result = graphService.buildFromCanonicalModel(canonicalModel);
      if (!result.success || !result.data) {
        res.status(400).json({
          error: result.error || "Failed to build graph from Canonical API Model.",
    ...
    // Reuses existing parser service -> CanonicalApiModel -> GraphBuilder
    const result = await graphService.buildFromSpec(specContent);

    if (!result.success || !result.data) {
      res.status(400).json({
        error: result.error || "Failed to build graph from specification.",
        details: result.details,
      });
      return;
    }
    ...
    res.status(500).json({
      error: `Internal server error while building API graph: ${err.message}`,
    });
  }
};